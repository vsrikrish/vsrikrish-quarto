...
-- pandoc.pipe's subprocess runs with the *document's* directory as its
-- cwd (cv/), not the project root, so relative paths here must be
-- resolved against quarto.project.directory to work regardless of which
-- page the shortcode is used from.
local function loadBib(path)
  if cache[path] then return cache[path] end
  local absPath = quarto.project.directory .. "/" .. path
  local jsonText = pandoc.pipe("quarto", {"pandoc", "-f", "bibtex", "-t", "csljson", absPath}, "")
...
local CATEGORIES = {
  papers      = {file = "cv/bibliography/library.bib",     kind = "article",
                 pred = function(e) return e.type == "article-journal" end},
  inpress     = {file = "cv/bibliography/library.bib",     kind = "inpress",
                 pred = function(e) return e.keyword == "unpublished" end},
  reports     = {file = "cv/bibliography/library.bib",     kind = "report",
                 pred = function(e) return e.type == "report" end},
  books       = {file = "cv/bibliography/library.bib",     kind = "book",
                 pred = function(e) return e.type == "book" end},
  conferences = {file = "cv/bibliography/library.bib",     kind = "conference",
                 pred = function(e) return e.type == "paper-conference" end},
  talks       = {file = "cv/bibliography/conferences.bib", kind = "talk",
                 pred = function(e) return e.keyword == "talk" end},
  posters     = {file = "cv/bibliography/conferences.bib", kind = "poster",
